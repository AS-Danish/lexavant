  ...
  return (
    <section className="bg-bone py-28 md:py-40">
      <div className="container">
        <div className="flex items-center gap-3 font-mono text-[11px] uppercase tracking-[0.3em] text-ink/60 mb-12" data-reveal>
          <span className="h-px w-8 bg-gold" /> 07 — In Their Words
        </div>

        <div className="grid lg:grid-cols-12 gap-12 items-end" key={i}>
          <blockquote className="lg:col-span-9 font-serif text-3xl md:text-5xl lg:text-6xl leading-[1.1] text-ink animate-fade-in">
          ...
          <div className="lg:col-span-3 lg:text-right animate-fade-in">
            <div className="font-serif text-xl text-ink">{quotes[i].a}</div>
            <div className="mt-1 font-mono text-[10px] uppercase tracking-[0.25em] text-ink/55">{quotes[i].t}</div>
          </div>
        </div>

        <div className="mt-16 flex items-center justify-between border-t border-ink/15 pt-6">
          <div className="font-mono text-[11px] uppercase tracking-[0.25em] text-ink/55">
            <span className="text-ink">{String(i + 1).padStart(2, "0")}</span>
            <span className="mx-2 text-ink/30">/</span>
            {String(quotes.length).padStart(2, "0")}
          </div>
          <div className="flex gap-2">
            <button onClick={prev} aria-label="Previous quote" className="h-12 w-12 grid place-items-center rounded-full border border-ink/20 hover:bg-ink hover:text-bone hover:border-ink transition-all duration-500">
              <ChevronLeft size={16} />
            </button>
            <button onClick={next} aria-label="Next quote" className="h-12 w-12 grid place-items-center rounded-full border border-ink/20 hover:bg-ink hover:text-bone hover:border-ink transition-all duration-500">
