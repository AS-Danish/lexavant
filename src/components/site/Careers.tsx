...
export const Careers = () => {
  return (
    <section id="careers" className="bg-ink-soft py-28 md:py-40">
      <div className="container">
        <div className="grid lg:grid-cols-12 gap-12 mb-16 items-end">
          <div className="lg:col-span-7">
            ...
            <h2 className="font-serif text-4xl md:text-6xl leading-[1.05] text-bone" data-reveal>
              Build a practice that
              <span className="italic text-gold-deep"> outlasts you.</span>
            </h2>
          </div>
          <p className="lg:col-span-4 lg:col-start-9 text-bone/70 leading-relaxed" data-reveal>
        ...
        <ul className="border-t border-bone/15" data-stagger>
          {roles.map((r) => (
            <li key={r.title} data-stagger-item>
              <a href="#contact" className="group grid grid-cols-12 items-center gap-4 py-7 border-b border-bone/15 hover:bg-ink transition-colors duration-500 px-2">
                <span className="col-span-12 md:col-span-2 font-mono text-[11px] uppercase tracking-[0.2em] text-gold-deep">{r.dept}</span>
                <h3 className="col-span-12 md:col-span-5 font-serif text-2xl md:text-3xl text-bone transition-transform duration-500 group-hover:translate-x-2">
                  {r.title}
                </h3>
                <span className="col-span-6 md:col-span-2 font-mono text-[11px] uppercase tracking-[0.18em] text-bone/60">{r.loc}</span>
                <span className="col-span-6 md:col-span-2 font-mono text-[11px] uppercase tracking-[0.18em] text-bone/60">{r.type}</span>
                <span className="col-span-12 md:col-span-1 justify-self-end text-bone/40 group-hover:text-gold transition-all group-hover:translate-x-1">
                  →
                </span>
              </a>
            </li>
